...
const inlineDiffMargin_1 = require("./inlineDiffMargin");
...
const workerBasedDocumentDiffProvider_1 = require("./workerBasedDocumentDiffProvider");
...
let DiffEditorWidget = class DiffEditorWidget extends lifecycle_1.Disposable {
    constructor(domElement, options, codeEditorWidgetOptions, clipboardService, contextKeyService, instantiationService, codeEditorService, themeService, notificationService, contextMenuService, _editorProgressService) {
        ...
        this._documentDiffProvider = instantiationService.createInstance(workerBasedDocumentDiffProvider_1.WorkerBasedDocumentDiffProvider);
    ...
    revertChange(change) {
        ...
        const originalRange = change.originalEndLineNumber > 0 ? new range_1.Range(change.originalStartLineNumber, 1, change.originalEndLineNumber, original.getLineMaxColumn(change.originalEndLineNumber)) : null;
        const originalContent = originalRange ? original.getValueInRange(originalRange) : null;
        const newRange = change.modifiedEndLineNumber > 0 ? new range_1.Range(change.modifiedStartLineNumber, 1, change.modifiedEndLineNumber, modified.getLineMaxColumn(change.modifiedEndLineNumber)) : null;
        ...
        if (change.originalEndLineNumber === 0 && newRange) {
            ...
            const insertAt = change.modifiedStartLineNumber < modified.getLineCount() ? new position_1.Position(change.modifiedStartLineNumber + 1, 1) : new position_1.Position(change.modifiedStartLineNumber, modified.getLineMaxColumn(change.modifiedStartLineNumber));
            editor.executeEdits('diffEditor', [{
                    range: range_1.Range.fromPositions(insertAt, insertAt),
...
const DECORATIONS = {
    arrowRevertChange: textModel_1.ModelDecorationOptions.register({
        description: 'diff-editor-arrow-revert-change',
        glyphMarginClassName: 'arrow-revert-change ' + themeService_1.ThemeIcon.asClassName(codicons_1.Codicon.arrowRight),
    }),
...
function createViewZoneMarginArrow() {
    ...
    arrow.className = 'arrow-revert-change ' + themeService_1.ThemeIcon.asClassName(codicons_1.Codicon.arrowRight);
...
function validateDiffEditorOptions(options, defaults) {
    return {
        ...
        diffAlgorithm: (0, editorOptions_1.stringSet)(options.diffAlgorithm, defaults.diffAlgorithm, ['smart', 'experimental']),
    };
}
